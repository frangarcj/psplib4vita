...
static int _callback_thread(SceSize args, void* argp)
{
  ...
  {
    cbid = sceKernelCreateCallback("Exit Callback", 0, _callback, &_exit_callback);
...
int pl_psp_start_callback_thread()
{
  ...
  if ((thid = sceKernelCreateThread("update_thread", _callback_thread, 0x10000100, 0xFA0, 0, 0,NULL)) < 0)
    return 0;
